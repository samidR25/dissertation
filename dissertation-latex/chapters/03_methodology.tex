...
This choice was made because consider what a stratified (randomly shuffled) split would permit: a five-second 
...
this way could score well by effectively recognising a near-duplicate of something it had already seen, rather than by actually generalising to unseen seizure activity, and no real 
...
\realfig{eeg_electrode_montage.png}{0.85}{Standard EEG electrode placement (10-20 system) for the 18-channel montage used in this project. Verified against a real channel dump from \texttt{chb01\_01.edf}: only FT9 and FT10 are extra electrodes outside the 18-channel set; the remaining three nominally "excluded" channel labels (P7-T7, FT10-T8, and a duplicate T8-P8) are re-pairings of electrodes already present in the montage, not additional physical electrode sites.}{fig:eeg-montage}
...
the model recognising a synthetic near-duplicate of something it had implicitly been shown, rather than true generalisation. I corrected this project-wide once 
...
seed-sensitivity discussion in Section~\ref{sec:results-c2} concerns variance in outcome \emph{across} different seed values, not a lack of within-run determinism as each 
...
chronological validation split for this patient would have been driven by just noise rather than any real seizure-relevant signal, had the naive approach been used.
...
its reported false-positive rate remain fully reported findings.
...
dissertation that constitutes a cross-patient generalisation test, and the only regime fairly comparable against \citet{ali2024}'s LOPO figures. The second regime, used 
...
grounds that a clinician or patient cares about whether a real seizure \emph{event} was flagged, not the count of individual, sub-second windows classified correctly within it.
Window-level metrics, particularly window specificity, are also reported throughout, and window specificity additionally underpins a supplementary diagnostic described later in this section. 
Stating "event-level" as the primary unit of reporting is not itself 
...
\subsection{A Note on a Supplementary Diagnostic}

A model that fires on almost every window it sees will register a high event sensitivity by simple virtue of never staying silent through a true seizure which is an artefact of near-permanent triggering, not real discrimination. During analysis, I additionally screened results against a supplementary, self-designed heuristic (based on window specificity and predicted-block length) intended to flag this artefact. This heuristic is not a standard or externally validated technique, and none of this dissertation's primary reported figures depend on it. As such, the results in Chapter~\ref{ch:results} are reported using the same event-level sensitivity and FP/hr metrics described above, unfiltered, so that they remain directly comparable to \citet{ali2024} and \citet{shoeb2009} on equal terms. The full definition of this supplementary diagnostic, and a labelled secondary analysis using it, are given in the Limitations and Future Work discussion in Chapter~\ref{ch:discussion} rather than presented here as if it were established methodology.
...
patients with different prevalence -- revealed that chb15's C1 ratio was 6.0$\times$ base rate (indicating decent discrimination, with the model's threshold simply misplaced), while chb13's ratio was only 1.3$\times$ (near-chance, indicating weak discrimination). This is close to the \emph{opposite} of the hypothesis I began this project with, which expected chb15 to be inert and chb13/chb16 to be cases of misplaced thresholding. 
...
\label{sec:compute-complexity}

Before presenting measured power figures (Section~\ref{sec:results-hw}), it is worth first establishing what the network is actually computing per inference, since a power figure in isolation says little without a sense of the computational load producing it. Each convolutional layer's multiply-accumulate (MAC) cost follows the standard complexity bound
\[
O(H_{\text{out}} \times W_{\text{out}} \times C_{\text{out}} \times K_h \times K_w \times C_{\text{in}}),
\]
the product of the number of output positions, the number of output channels, and the number of multiply-accumulates each output position requires (kernel area times input channels); a dense layer's cost is simply $O(N_{\text{in}} \times N_{\text{out}})$, every input connected to every output once. These are upper bounds on the ANN's dense computational cost prior to quantisation or spiking conversion and Section~\ref{sec:results-hw} returns to why the deployed SNN's \emph{actual} dynamic energy draw does not follow directly from this figure, given the AKD1000's event-driven execution model (Chapter~\ref{ch:background}).

\subsection{Power Measurement Methodology}

Two different power figures are reported in this dissertation, and neither is presented as validating or superseding the other. The first is a chip-isolated estimate 
